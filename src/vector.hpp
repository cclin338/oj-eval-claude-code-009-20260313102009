...
private:
	...
	// Deallocate raw memory without calling destructors
	void deallocate(T* p) {
		::operator delete(p);
	}

	// Construct object at given position
	void construct(T* p, const T& value) {
		new (p) T(value);
	}

	// Destroy object at given position
	void destroy(T* p) {
		p->~T();
	}
...
public:
	...
	iterator insert(const size_t &ind, const T &value) {
		...
		if (size_ == capacity_) {
			...
			// Insert new element
			construct(new_data + ind, value);

			// Copy elements after insertion point
			for (size_t i = ind; i < size_; ++i) {
				construct(new_data + i + 1, data_[i]);
			}
			...
			for (size_t i = size_; i > ind; --i) {
				construct(data_ + i, data_[i - 1]);
				destroy(data_ + i - 1);
			}
			construct(data_ + ind, value);
		}
